...
public:
	...
	template<typename T>
	T* SpawnPoolActor(UClass* Class, FVector location, FRotator rotation) 
	{
		return Cast<T>(SpawnPooledActor(Class, location, rotation));
	}

	//오브젝트 풀을 이용한 스폰함수
	UFUNCTION(BlueprintCallable, Category = "ObjectPool")
	AActor* SpawnPooledActor(UClass* Class, FVector location, FRotator rotation);
